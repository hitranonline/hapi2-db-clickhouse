...
def __insert_base_items_core__(cls,ITEM_DICTS,local=True):
    ...
    if ITEM_DICTS_2: # IN CLICKHOUSE, THIS BLOCK IS UNREACHABLE !!!
    ...
    print('\nDEDUPLICATING TABLE "%s"'%cls.__table__)
    session.execute(text_('optimize table %s final deduplicate;'%cls.__table__))
...
def __insert_alias_items_core__(cls,ITEM_DICTS,local=True):
    ...
    # ---> update existing transitions (for Clickhouse, use insert instead)
    if ITEM_DICTS_2:
        #print('ITEM_DICTS_2>>>',json.dumps([{b:e[b] for b in e if b not in ['__parent__','__parents__']} for e in ITEM_DICTS_2],indent=3))
        #print('ITEM_DICTS_2>>>',ITEM_DICTS_2)
        session.execute(cls.__table__.insert(),ITEM_DICTS_2)
